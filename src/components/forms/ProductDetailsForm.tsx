...
export function ProductDetailsForm({
  ...
  const handleUpdate = (router : any, pathname : any) => {
    formData.productCapacity = '1'
    AxiosService.put(`product/updateProductDetails/${formData.productId}`,{
      success : (data : any) => {
      ...
      failed : (data : any) => {
        console.log("failed updation",data)
      ...
      error : (data : any) => {
        console.log("error in updation",data)
        ...
        setSelectedOption(
          formData.productCategoryId !== 0
            ? responseOptions.find(
                (option: any) => option.id === formData.productCategoryId
              )?.name || ""
            : ""
        );
      },
  ...
  const checkIsNotEmpty = () => {
    ...
    if (
      !formData.advanceBookingDuration ||
      formData.advanceBookingDuration < 1
    ) {
      emptyFields.push(
        "Advance Booking Duration cannot be empty and less than one"
      );
    }
